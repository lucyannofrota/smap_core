...
namespace smap
{
      ...
      std::shared_ptr<smap_interfaces::srv::AddPerceptionModule::Response> response)
  {
    ...
    if (request->type == "object")
      det.type = detector_type::object;
    else
    {
      if (request->type == "place")
        det.type = detector_type::place;
      else
      {
        response->success = false;
        return;
    ...
    if(
      (det.classes.empty()) ||
      (det.n_classes != det.classes.size())
    ){
      response->success = false;
    ...
    for (auto detector : this->detectors)
    {
      if (detector.name == request->name)
      {
        ...
        response->success = (
          ...
          (detector.architecture == det.architecture) &&
        ...
        if(response->success){
          ...
          RCLCPP_WARN(this->get_logger(),"Client requesting connection with duplicate detector name. The id of the first entry will be sent.");
        }
        else RCLCPP_WARN(this->get_logger(),"Client requesting connection with duplicate detector name and different classes.");
        return;
      }
    }
  ...
  void perception_server::print_classes(std::string pref, std::list<std::pair<int,std::string>> &classes)
  {
    ...
    {
      RCLCPP_INFO(this->get_logger(),"%s%4i| %s",
             pref.c_str(),
             cls.first,
             cls.second.c_str());
    }
  ...
  {
    RCLCPP_INFO(this->get_logger(),"%sDet:\n%s\tarch: %s\n%s\tname: %s\n%s\tid: %i\n%s\ttype: %i\n%s\tn_classes: %i\n%s\tclasses:",
           pref.c_str(),
           pref.c_str(), det.architecture.c_str(),
           pref.c_str(), det.name.c_str(),
           pref.c_str(), det.id,
           pref.c_str(), det.type,
           pref.c_str(), (int)det.n_classes,
           pref.c_str());
